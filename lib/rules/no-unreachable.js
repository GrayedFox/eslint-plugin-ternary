      ...
      ConditionalExpression(node) {
        ...
        for (const conditionNode of conditionNodes) {
          ...
          if (isDuplicate(condition) || isInvertedDuplicate(invertedCondition)) {
            reportNode(node, 'duplicateCondition', { expression: condition })
          }
          if (isInvertedDuplicate(condition) || isDuplicate(invertedCondition)) {
            reportNode(node, 'duplicateInvertedCondition', { expression: condition })
          }
          if (!isAndChild(conditionNode) && isOrEquivalent(condition)) {
            reportNode(node, 'equivalentOrCondition', { expression: condition })
          } else if (
            !isAndChild(conditionNode) &&
            !allowDuplicateOrConditions &&
            isOrDuplicate(condition)
          ) {
            reportNode(node, 'duplicateOrCondition', { expression: getOrDuplicate(condition) })
          }
